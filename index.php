...
head
  ...
  titleDashboard Admin Keuangantitle
  script src=httpscdn.tailwindcss.comscript
  link
    rel=stylesheet
    href=httpscdnjs.cloudflare.comajaxlibsfont-awesome5.15.3cssall.min.css
  ...
  div class=max-w-5xl mx-auto w-full flex flex-col bg-white shadow-lg rounded-lg border border-gray-200 overflow-hidden my-8
    ...
    header class=flex justify-between items-center bg-white border-b border-gray-200 px-6 py-4
      div class=flex items-center space-x-3
        div class=w-10 h-10 bg-indigo-600 rounded flex items-center justify-center text-white font-bold text-lg select-noneLdiv
        h1 class=text-xl font-semibold tracking-wideDashboard Admin Keuanganh1
      div
      button class=text-indigo-600 font-semibold hovertext-indigo-800 transitionLogoutbutton
    header

    !-- Controls --
    section class=flex flex-col smflex-row justify-between items-center gap-4 px-6 py-4 border-b border-gray-200
      div class=flex items-center space-x-3 w-full smw-auto
        label for=jenisSelect class=font-medium text-gray-700 whitespace-nowrapPilih Jenislabel
        select id=jenisSelect class=border border-gray-300 rounded-md bg-white text-gray-900 text-sm font-mono px-3 py-1.5 focusoutline-none focusring-2 focusring-indigo-500 focusborder-indigo-500 transition w-40
          option value=gajiGajioption
          option value=tppTPP Voption
        select
      div
      div class=text-gray-600 font-medium text-sm whitespace-nowrap flex items-center space-x-2
        label for=periodeInput class=font-monoPeriodelabel
        input
          type=month
          id=periodeInput
          class=border border-gray-300 rounded-md bg-white text-gray-900 text-sm font-mono px-3 py-1.5 focusoutline-none focusring-2 focusring-indigo-500 focusborder-indigo-500 transition
          value=2025-09
          aria-label=Periode
    ...
    !-- Ringkasan Progress --
    section class=px-6 py-4 border-b border-gray-200 flex flex-col smflex-row justify-between gap-4 text-sm font-mono
      div
        h2 class=text-gray-900 font-semibold mb-2Ringkasan Progressh2
        ul class=list-disc list-inside space-y-1 text-gray-700
          liGaji September 2025li
          liTPP Juli 2025li
        ul
      div
    ...
    section id=tahapanDescription class=px-6 py-4 border-b border-gray-200 text-xs text-gray-700 whitespace-pre-line font-mono leading-relaxed max-h-48 overflow-y-autosection

    !-- Detail Tahapan --
    section class=px-6 py-4 border-b border-gray-200 text-xs font-mono
      h3 id=detailTitle class=font-semibold mb-2Detail Tahapan GAJI - September 2025h3
      div class=overflow-x-auto
        table class=min-w-full border border-black border-collapse text-xs
          thead
            tr class=border border-black bg-gray-100
              th class=border border-black px-1 text-center w-8Noth
              th class=border border-black px-1 text-leftTahapanth
              th class=border border-black px-1 text-center w-16Statusth
              th class=border border-black px-1 text-center w-20Updateth
            tr
          thead
          tbody id=tahapanTableBody class=bg-whitetbody
        table
      div
    section
    ...
    !-- + Update Tahapan --
    button
      class=w-full text-indigo-600 font-semibold text-left px-6 py-2 hoverbg-indigo-50 transition flex items-center space-x-2 border-b border-gray-200 font-mono
      id=toggleUpdateForm
      aria-expanded=false
      aria-controls=updateForm
    
      i class=fas fa-plusi
      span+ Update Tahapanspan
    button

    !-- Update Tahapan form --
    section id=updateForm class=px-6 py-4 text-xs text-gray-700 font-mono hidden
      div class=mb-2 font-semiboldUpdate Tahapandiv
      form onsubmit=return false; class=space-y-2
        div
          label for=inputTahapan class=block mb-0.5Tahapan label
          input
            type=text
            id=inputTahapan
            readonly
        ...
        div
        div
          label for=inputStatus class=block mb-0.5Status label
          input
            type=text
            id=inputStatus
            readonly
        ...
        div
        div
          label for=inputTanggal class=block mb-0.5Tanggal label
          input
            type=text
            id=inputTanggal
            readonly
        ...
        div
        div
          label for=inputCatatan class=block mb-0.5Catatan label
          textarea
            id=inputCatatan
            readonly
            rows=2
        ...
        div
        div
          label for=inputDokumen class=block mb-0.5Dokumen label
          input
            type=text
            id=inputDokumen
            readonly
        ...
        div class=flex space-x-2
          button
            type=button
            class=border border-black px-3 py-1 font-semibold hoverbg-gray-200 transition
...
Transfer ke rekening Pegawai – Bank Kalbar`;
    ...
    const gajiRows = [
      { no 1, tahapan Penerbitan daftar gaji – BKD, status Done, update 2708 },
      { no 2, tahapan Pendistribusian daftar gaji ke UPT & RSUD – Subbag Keuangan dan Perlengkapan, status Done, update 2708 },
      { no 3, tahapan Verifikasi daftar transfer rekening gaji – UPT & RSUD, status Done, update 2708 },
      { no 4, tahapan Kompilasi daftar transfer rekening gaji – Subbag Keuangan dan Perlengkapan, status Done, update 2708 },
      { no 5, tahapan Pembuatan template gaji pada aplikasi – Subbag Keuangan dan Perlengkapan, status Done, update 2808 },
      { no 6, tahapan Pembuatan SPP & SPM gaji – Subbag Keuangan dan Perlengkapan, status Proses, update 3008 },
    ...
    const tppRows = [
      { no 1, tahapan Laporan daftar absensi (softfile & hardfile) – UPT & RSUD, status Done, update 2708 },
      { no 2, tahapan Verifikasi laporan daftar absensi – Subbag Umum Aparatur, status Done, update 2708 },
      { no 3, tahapan Penyampaian laporan daftar absensi ke BKD – Subbag Keuangan & Perlengkapan, status Done, update 2708 },
      { no 4, tahapan Penerbitan daftar TPP – BKD, status Done, update 2708 },
      { no 5, tahapan Kompilasi daftar transfer rekening TPP – Subbag Keuangan & Perlengkapan, status Done, update 2708 },
      { no 6, tahapan Pembuatan template TPP pada aplikasi – Subbag Keuangan & Perlengkapan, status Done, update 2808 },
      { no 7, tahapan Pembuatan SPP & SPM TPP – Subbag Keuangan & Perlengkapan, status Proses, update 3008 },
    ...
    function createStatusCell(status) {
      ...
      return `td class=border border-black px-1 text-center ${color} font-semibold flex justify-center items-center space-x-1
                i class=fas ${icon}ispan${status}span
              td`;
    }
    ...
    function updateContent() {
      ...
      const periodeFormatted = formatPeriode(periode);
       ...
       Update periode text
      document.querySelector('section.flex.flex-col.smflex-row  div.text-gray-600.font-medium.text-sm').innerHTML =
